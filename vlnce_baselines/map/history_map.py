...
class HistoryMap(nn.Module):
    ...
    def _draw_polyline(self, trajectory_points: List, thickness: int=20):  # 把轨迹点序列画成“粗线”mask
        image = np.zeros(self.shape)
        points_array = np.array(trajectory_points, dtype=np.int32) # 轨迹点转 int32，满足 cv2.polylines 输入
        color = (255, 255, 255)
        is_closed = False
        
        # thicness = 20 => 1.0(meter) takes 20 grids in map
        trajectory = cv2.polylines(image, [points_array], is_closed, color, thickness)# 在 image 上画折线（
        
        return trajectory.astype(bool)
    
    def forward(self, trajectory_points: List, step: int, current_episode_id: int, alpha: float=0.95): # 更新历史惩罚图：把当前轨迹覆盖区域乘以 alpha（衰减）
        if len(trajectory_points) == 2 and trajectory_points[0] == trajectory_points[1]:
            return self.history_map
        trajectory = self._draw_polyline(trajectory_points) # 生成轨迹覆盖 mask（bool）
        self.history_map[trajectory == True] *= alpha # 对走过区域做衰减（累乘）：走得越多值越小
